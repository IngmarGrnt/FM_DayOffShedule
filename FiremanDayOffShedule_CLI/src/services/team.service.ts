...
console.log('getTeams', this.http.get<{ $id: string, $values: { id: number, name: string }[] }>(this.apiUrl));
  ...
  getWorkDaysForYear(id: number, year: number): Observable<TransformedWorkDays> {
    return this.http.get<WorkDays>(`${this.apiUrl}/workdays/${id}/${year}`).pipe(
      map((data) => ({
